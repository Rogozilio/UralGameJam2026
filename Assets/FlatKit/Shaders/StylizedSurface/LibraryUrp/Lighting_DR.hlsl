...
half3 LightingPhysicallyBased_DSTRM(Light light, InputData inputData)
{
    ...
    const float gradientY = inputData.positionWS.y;
#else
    const float3 localPosition = TransformWorldToObject(inputData.positionWS);
    const float2 position = localPosition.xy;
    const float gradientY = localPosition.y;
#endif
    const float gradientSizeSafe = max((float)_GradientSize, 0.0001);
    const float posGradRotated = (position.x - _GradientCenterX) * sin(angleRadians) + 
                                 (position.y - _GradientCenterY) * cos(angleRadians);

    half gradientFactor = saturate((_GradientSize * 0.5 - posGradRotated) / gradientSizeSafe);
    gradientFactor = lerp(gradientFactor, 1.0h - gradientFactor, saturate(_GradientInvert));

    const half gradientY01 = saturate(((half)gradientY - _GradientCenterY) / (half)gradientSizeSafe + 0.5h);
    half topDownCoord = 1.0h - gradientY01;
    topDownCoord = lerp(topDownCoord, 1.0h - topDownCoord, saturate(_GradientInvert));
    const half fillMask = smoothstep((1.0h - _GradientFill) - _GradientSoftness,
                                     (1.0h - _GradientFill) + _GradientSoftness,
                                     topDownCoord);

    gradientFactor *= fillMask;
...
half4 UniversalFragment_DSTRM(InputData inputData, SurfaceData surfaceData, float2 uv)
{
    ...
    */
    ...
    half finalAlpha = surfaceData.alpha;
#if defined(DR_GRADIENT_ON)
    #if defined(_GRADIENTSPACE_WORLD)
    const float alphaGradientY = inputData.positionWS.y;
    #else
    const float alphaGradientY = TransformWorldToObject(inputData.positionWS).y;
    #endif
    const float alphaGradientSizeSafe = max((float)_GradientSize, 0.0001);
    const half alphaGradientY01 = saturate(((half)alphaGradientY - _GradientCenterY) / (half)alphaGradientSizeSafe + 0.5h);
    half alphaTopDownCoord = 1.0h - alphaGradientY01;
    alphaTopDownCoord = lerp(alphaTopDownCoord, 1.0h - alphaTopDownCoord, saturate(_GradientInvert));
    const half alphaFillMask = smoothstep((1.0h - _GradientFill) - _GradientSoftness,
                                          (1.0h - _GradientFill) + _GradientSoftness,
                                          alphaTopDownCoord);
    finalAlpha *= alphaFillMask;
#endif
