...
Shader "Vefects/SH_Vefects_URP_VFX_Opaque_Fr_01"
{
	Properties
	{
		...
		[Space(20)][Header(Gradient Overlay)] _GradientFill( "Gradient Fill Top -> Down", Range( 0, 1 ) ) = 1
		_GradientSoftness( "Gradient Softness", Range( 0.001, 0.5 ) ) = 0.1
		[Toggle( _GRADIENT_OBJECTSPACE_ON )] _GradientObjectSpace( "Gradient In Object Space", Float ) = 1
		_GradientObjectMinY( "Object Gradient Min Y", Float ) = -0.5
		_GradientObjectMaxY( "Object Gradient Max Y", Float ) = 0.5
		_GradientInvert( "Invert Gradient", Range( 0, 1 ) ) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature_local_fragment _GRADIENT_OBJECTSPACE_ON
			...
			float _GradientObjectMinY;
			float _GradientObjectMaxY;
						 ...
						 ) : SV_Target
			{
				...
				#endif
				float topDownGradientCoord = ( 1.0 - gradientCoord );
				topDownGradientCoord = lerp( topDownGradientCoord, ( 1.0 - topDownGradientCoord ), saturate( _GradientInvert ) );
				float gradientMask = smoothstep( ( ( 1.0 - _GradientFill ) - _GradientSoftness ) , ( ( 1.0 - _GradientFill ) + _GradientSoftness ) , topDownGradientCoord );
				float maskedIntensity = saturate( temp_output_64_0 * gradientMask );
				float3 lerpResult25 = lerp( _Color.rgb , _ColorFresnelBase.rgb , maskedIntensity);
				
				float2 temp_cast_1 = (_LUTSpeed).xx;
				float2 temp_cast_2 = (( ( maskedIntensity * _LUTAmplitude ) + _LUTOffset )).xx;
				...
				float Occlusion = 1;
				float3 Emission = ( staticSwitch62 * ( maskedIntensity * _FresnelEmissiveMultiply ) ).rgb;
				float Alpha = maskedIntensity;
